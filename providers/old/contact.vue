...
export default {
    ...
    async editAboutText(){
        ...
        const res = await this.callApi('post','editContactText',this.edit_data)
        
        if(res.status == 200){
          
            this.data[this.editIndex] = res.data.contact
 ...
 async created(){
   ...
   const res = await this.callApi('get',`showContactText`)
        	if( res.status != 200){
